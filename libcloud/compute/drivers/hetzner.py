...
from libcloud.common.types import ServiceUnavailableError, ProviderError
...
class rate_limited:
    """
    Decorator for retrying Hetzner calls that are rate-limited.
...
class HetznerResponse(JsonResponse):
    def parse_error(self):
        if self.status == httplib.OK:
            ...
            raise ProviderError(self.body)
        else:
            raise LibcloudError(self.body)


class SSHKey(object):
    def __init__(self, id, finger_print, pub_key):
        self.id = id
        self.finger_print = finger_print
        self.pub_key = pub_key

    def __repr__(self):
        return (('<SSHKey: id=%s, finger_print=%s, pub_key=%s>') %
                (self.id, self.finger_print, self.pub_key))


class HetznerConnection(ConnectionUserAndKey):
    """
    Connection class for the Hetzner driver.
    """

    host = 'robot-ws.your-server.de'
    allow_insecure = False

    responseCls = HetznerResponse
    ...
    def delete(self, url, data):
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        return self.request(url, data=data, headers=headers, method='DELETE')


class HetznerRobotDriver(NodeDriver):
    """
    HetznerNode node driver.
    """

    connectionCls = HetznerConnection
    ...
    def list_nodes(self):
        return self._list_resources('/server', self._to_node)
    ...
    def list_key_pairs(self):
        ...
        """
        return self._list_resources('/key', self._to_ssh_key)
    ...
    def create_key_pair(self, name, public_key=''):
        ...
        res = self.connection.post('/key', params)
    ...
    def delete_key_pair(self, key_pair):
        ...
        res = self.connection.delete('/key/{}'.format(key_pair.finger_print))
        return res.status == httplib.OK

    def list_sizes(self):
        return self._list_resources('/order/server/product', self._to_size)
    ...
    def create_node(self, name, size, image, location, ex_ssh_key_ids=None,
        ...
        * `tag`: ``str`` The tag to assign to this server
        ...
        result = self.connection.post('/order/server/transaction', params)
    ...
    def reboot_node(self, node):
        res = self.connection.post('/reset/{}'.format(node.public_ips[0]))

        return res.status == httplib.OK

    def destroy_node(self, node):
        res = self.connection.post('/server/{}/cancellation'.format(node.public_ips[0]))
    ...
    def _to_node(self, data):
        data = data['server']
        data = self.connection.get('/server/{}'.format(data['server_ip'])).object
        data = data['server']
        if 'status' in data:
        ...
        if 'server_ip' in data and data['server_ip'] is not None:
            public_ips = [data['server_ip']]
        else:
            public_ips = []
        ...
        extra_keys = ['traffic', 'dc', 'paid_until', 'cancelled', 'throttled', 'reset', 'rescue', 'vnc',
                      'plesk', 'windows', 'flatrate']
        extra = {}
        for key in extra_keys:
            if key in data:
                extra[key] = data[key]

        node = Node(id=data['server_number'], name=data['server_name'], state=state,
