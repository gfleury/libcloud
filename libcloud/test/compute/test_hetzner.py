...
from libcloud.compute.drivers.hetzner import HetznerRobotDriver
...
# class HetznerTests(unittest.TestCase, TestCaseMixin):
class HetznerTests(LibcloudTestCase):

    def setUp(self):
        HetznerRobotDriver.connectionCls.conn_class = HetznerMockHttp
        HetznerMockHttp.type = None
        self.driver = HetznerRobotDriver(*VULTR_PARAMS)
    ...
    def test_rate_limit(self):
        HetznerMockHttp.type = 'SERVICE_UNAVAILABLE'
        self.assertRaises(ServiceUnavailableError, self.driver.list_nodes)


class HetznerMockHttp(MockHttp):
    fixtures = ComputeFileFixtures('hetzner')
